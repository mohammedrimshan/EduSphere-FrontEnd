...
export default function Profile() {
  ...
  const handleVerifyOtp = async (otpValue) => {
    ...
    try {
      ...
      toast.error(error.response?.data?.message || "Failed to verify OTP or update email");
    } finally {
      setIsSubmitting(false);
    }
  };
            ...
            <div className={`rounded-3xl shadow-xl overflow-hidden ${theme === 'dark' ? 'bg-gray-700' : 'bg-white'}`}>
              <div className="md:flex">
                <div className="md:w-1/2 p-6 md:p-8">
                  <h2 className={`text-2xl md:text-3xl font-bold mb-6 ${theme === 'dark' ? 'text-green-400' : 'text-green-800'}`}>Profile Management</h2>
                ...
                <div className={`md:w-1/2 p-6 md:p-8 ${theme === 'dark' ? 'bg-gray-600' : 'bg-green-50'}`}>
                  <h3 className={`text-xl md:text-2xl font-semibold mb-6 ${theme === 'dark' ? 'text-green-400' : 'text-green-800'}`}>Profile Picture</h3>
